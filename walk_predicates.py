...
class WalkPredicates(astor.TreeWalk):
    ...
    def pre_FunctionDef(self):
        trace = ast.copy_location(ast.arg(arg='trace', annotation=None), self.cur_node)
    ...
    def _inject_trace_hook(self, compare_node, branch_id):
        ...
        f_call = 'trace.{fname}({branch_id}, {lhs}, {rhs})'.format(fname=op, branch_id=branch_id, lhs=lhs, rhs=rhs)
    ...
    def pre_If(self):
        ...
        self.cur_node.test = self._inject_trace_hook(self.cur_node.test, self.cur_branch_id)

        true_branch_node = AnyNode(id=self.cur_branch_id, type=True, ast_node=self.cur_node, parent=parent)
        false_branch_node = AnyNode(id=self.cur_branch_id, type=False, ast_node=self.cur_node, parent=parent)
